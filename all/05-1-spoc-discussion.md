...
每个字段具体的含义和作用在注释里都有写, 就不必多说了.

为了更好理解各个字段的关系, 不妨将 PCB 中字段分为下面的类型

* 描述进程自身信息的.
  - 含 `pid`, `name`, `state`, `flags`, `exitcode`.

* 描述和其他进程关系的, 以及 ucore 用来组织所有进程的.
  - 含 `parent`, `hash_link`, `list_link`, `cptr`, `yptr`, `optr`.

* *lab1 的内容在 PCB 中*: 和用户内核态切换相关的 e.g. 不严密地说, 和中断以及 syscall 相关的.
  - 含 `kstack` 和 `tf`.

* *lab23 的内容在 PCB 中*: 和内存管理相关的.
  - 含 `mm` 和 `cr3`.

* *lab6 的内容在 PCB 中的*: 和进程调度等相关的.
  - 含 `runs`, `need_resched`, `wait_state`.

当然, 到 lab78 之后 PCB 中包含的信息会越来越多.
但是如果按照上面的分类来理解这些字段, 会更好抓住它们之间的联系.
